...
export default function AnalyticsTab({
  ...
  // Concept mastery helpers
  const attemptNumbers = [
    ...new Set(conceptMastery.map((m) => m.attempt_number)),
  ].sort((a, b) => a - b);
  const conceptTags = [...new Set(conceptMastery.map((m) => m.concept_tag))];
  ...
  return (
    <div className="space-y-4">
      ...
      {conceptTags.length > 0 && (
        <div className="bg-white shadow-sm rounded-md p-6">
          <h3 className="text-base font-medium text-zinc-900 mb-4">Concept Mastery</h3>
