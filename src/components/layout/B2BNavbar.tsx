      ...
      <div className="max-w-6xl mx-auto px-4 md:px-8">
        <div className="h-14 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3 shrink-0">
            ...
            <span className="text-sm font-semibold text-zinc-900 hidden sm:block">
              {tenantInfo?.name ?? tenantSlug}
            </span>
          </div>

          {/* Right — learner avatar + logout */}
          <div className="flex items-center gap-3 shrink-0">
            <div className="w-8 h-8 rounded-full bg-teal-100 text-teal-700 text-xs font-semibold flex items-center justify-center">
              {getInitials(learner.full_name)}
            </div>
            <span className="hidden sm:block text-sm text-zinc-600 max-w-[120px] truncate">
              {learner.full_name}
            </span>
            <button
